...
//			duplicateRow = 0;
				...
				if (e.getSapId() != null) {
					...
					if (employeeDetailsRepository.findById(e.getSapId()).equals(Optional.empty())) {
						...
						if (!Constraints.mobileNumberValidate(e.getContactNo().toString())) {
							errorsList.add("Contact number is not Correct at row" + rowNumber + " in non-tier1 excel sheet");
							flag = true;
						}
						...
						if (l1Repository.findByValue(e.getL1()) == null) {
							errorsList.add(
									"values are null or improper in row " + rowNumber + " in L1 column of non-tier1 excel sheet");
							flag = false;
						}
						if (l2Repository.findByValue(e.getL2()) == null) {
							errorsList.add(
									"values are null or improper in row " + rowNumber + " in L2 column of non-tier1 excel sheet");
							flag = false;
						}
						if (l3Repository.findByValue(e.getL3()) == null) {
							errorsList.add(
									"values are null or improper in row " + rowNumber + " in L3 column of non-tier1 excel sheet");
							flag = false;
						}
						if (l4Repository.findByValue(e.getL4()) == null) {
							errorsList.add(
									"values are null or improper in row " + rowNumber + " in L4 column of non-tier1 excel sheet");
							flag = false;
						}
						if (flag == true) {
					...
					errorsList
							.add("sap-id is null or improper in row no " + rowNumber + " in the non-tier1 excel sheet");
				}
				rowNumber++;
			...
			responseList.setFailed_Records(employeeNonTier1List.size()-employeeDetailsList.size());
